...
int f(int x){ // definição da funcao f
    int y;
    y = x+1;
    return y;
}
...
```
 ...
 - ```nome_da_funcao``` identifica a função e permite chamá-la pelo nome no decorrer do código-sonte.
 - Parâmetros (opcional): valores que podem ser passados para a função, para que ela os utilize em seu código-fonte.
 - Corpo: O bloco de código-fonte entre abre-chaves que implementa os comandos, operações e instruções da função.

Então não é necessário colocar o comando ```return```? Exatamente. Em funções **void,** não é necessário colocar o comando ```return```. Em alguns tipos de funções, utiliza-se o ```return ;``` sem explicitar o valor de retorno.
...
![passagem_valor2](https://github.com/Leppyzin/competitive_programming/blob/main/Arquivos_img/endere%C3%A7o2.png)
...
![passagem_valor3](https://github.com/Leppyzin/competitive_programming/blob/main/Arquivos_img/endere%C3%A7o3.1.png)
...
![passagem_valor5](https://github.com/Leppyzin/Programacao_Competitiva/blob/main/Arquivos_img/passagem_valor_05.png)
