...
## Cálculo do Desvio Padrão Populacional
 ...
 Esse código em C calcula o desvio padrão populacional de um conjunto de números inteiros fornecidos pelo usuário.
 De maneira geral, o Pseudo-código pode ser descrito como:

  1) Lê um número n, que representa a quantidade de elementos que o usuário vai fornecer.
  2) Lê n inteiros e armazena na amostra ```va``` (ou vetor ```va[]```).
  3) Calcula a média dos valores.
  4) Calcula o desvio padrão populacional dos valores.
  5) Imprime o desvio padrão com duas casas decimais de precisão.
...
A implementação em linguagem C ocorre como:
 ...
 - Temos também três variáveis do tipom double ```soma```, ```media``` e ```std``` que servem para somar valores, armazenar o valor da média dos elementos do vetor e armazenar o desvio padrão, respectivamente.
...
## Busca Sequencial em Vetor
 ...
 Uma busca sequencial (ou busca linear) é um algoritmo simples que encontra um determinado valor em uma coleção de dados (como um vetor). A ideia desse algoritmo consiste em percorrer os elesmentos do vetor, um por um, início ao fim, e em cada elemento acessado, verificar se ele é igual ao elemento que está sendo buscado.

 Seja um valor ```x``` a chave de busca e a coleção de elementos representada em um vetor ```v```(```v[0], v[1], ... , v[n-a]```), isto é, o elemento que estmaos querendo encontrar no vetor. Faça:

  1) A cada passo ```i```, compara o valor atual ```v[i]``` com o valor procurado ```x```.
  2) Se encontrar ```x``` (```v[i] == x``` retornar verdadeiro), retorna a sua posição, isto é, o índice ```i```. Caso contrário, analise o próximo elemento, isto é, ```i = i+1```
  3) Se chegar ao final do vetor sem encontrar ```x```, indica que o valor não está presente.

 A implementação em Linguagem C está apresentada abaixo. Para um vetor ```v``` com capacidade máxima de 2000 elementos, e contendo ```n``` elementos, verificamos se ```x``` está ou não presente no vetor ```v```. Caso o elemento esteja presente, essa implementação sempre retorna a primeira ocorrência de ```x``` em ```v```.
