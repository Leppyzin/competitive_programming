# Vetores e Funções
 ...
 Uma função é um bloco de código-fonte que realiza uma tarefa específica. Geralemente, uma função é declarada e definida uma **única** vez e pode ser chamada várias vezes ao longo do programa.

 A Linguagem C permite que vetores sejam passados como argumento das funções. Entretanto há algumas peculiaridades a serem detalhadas adiante.

 ## Relembrando a Passagem por Valor

  A passagem por valor na Linguagem C ocorre quando uma cópia do valor de um variável é enviada para uma função. Nesse caso, qualquer modificação feita no parâmetro dentro da função não afeta a variável original, pois a função trabalha apenas com a cópia do valor.
...
```
 ...
 Observe que os valores armazenados nas variáveis ```n``` e ```m``` são copiados para as variáveis ```a``` e ```b```, respectivamente. Observe também que os endereços de memória dessas variáveis são diferentes, com as variáveis ```n``` e ```m``` sendo relacionadas com a função ```main``` e as variáveis de quaisquer tipos nativos (int,float,double,char) são passadas por valor.
 Mas considerando os vetores, essa história muda...
...
## Exemplos
 ...
 Matematicamente, seja um vetor ```a``` contendo os ```n``` elementos ```a[0],a[1],a[2], ..., a[n-1]```. Podemos definir a doma acumulada como:
...
void somaAcumulada(long long int[5], int n){
    int i;
    for(i = 1; i < n; i++){
        a[i] = a[i]+a[i-1];
    }
...
4 5 15 18 20
```
**Homework:** se eu quisesse guardadr a soma acumulada do vetor ```a``` em outro vetor?
