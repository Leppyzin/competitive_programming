...
```
 ...
 Aqui, o vetor ```str``` tem espaço para até 10 caracteres. A constante ```"Ola mundo"``` ocupa 9 posições + o caractere nulo ```\```.
...
```
 ...
 é importante salientar que ```scanf``` com o especificador ```%s``` não lê espaços em branco. Caso você digite *"One Piece"* e apertar
 *Enter*, aparecerá na saída:
...
```
 ...
 Observe que se o usuário digitar mais caracteres do que a string ```str1``` suporta, a memória adjacente associada à outra variável ```str2``` será sobrescrita. Por isso, tome cuidado ao declarar o tamanho máximo de cada string. **Sempre coloque o espaço de memória realmente necessário para armazenar todos os caracteres a serem lidos da entrada em uma string.** Em juízes online, como o Beecrowd, verifique nos limites das entradas do problema, o tamanho máximo que cada string pode possuir.
...
```
 ...
 Ela garante que não mais do que o tamanho do vetor seja lido. Além disso, ela inclui o caractere de nova linha (```\n```) na string, a menos que a linha lida seja maior que o espaço disponível.
